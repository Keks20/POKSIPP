...
// Uvoz fraDnevnikMusterije OVDE - u implementation da nema kružne reference
uses fraDnevnikMusterije;
...
begin
  ...
  try
    ...
    // Ako je musterija - ucitaj samo njene ljubimce
    if LoggedInRole = 'MUSTERIJA' then
    begin
      Q.SQL.Text :=
        'SELECT id, name, species, breed, age, image_blob ' +
        'FROM pets ' +
        'WHERE Sifra_musterije = :mid ' +
        'ORDER BY id';
      Q.ParamByName('mid').AsInteger := LoggedInUserID;
    end
    else
    begin
      // Osoblje i ostali vide sve ljubimce
      Q.SQL.Text :=
        'SELECT id, name, species, breed, age, image_blob ' +
...
// -------------------------------------------------------
//  IZMENJENO: Klik na ljubimca
//  - Musterija → otvara fraDnevnikMusterije
//  - Osoblje/ostali → osvezava karticu kao i ranije
// -------------------------------------------------------
...
begin
  ...
  SelectedPetID  := Pets[ActivePetIndex].Id;
  ListBox1.Visible := False;

  if LoggedInRole = 'MUSTERIJA' then
  begin
    // Musterija klikne ljubimca → ide na dnevnik aktivnosti
    TNavFrames.Go(TFrameDnevnikMusterije.Create(nil));
  end
  else
  begin
    // Osoblje i ostali → samo osvezi karticu
    RefreshMainCard;
  end;
end;
